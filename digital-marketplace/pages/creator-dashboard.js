...
export default function CreatorDashboard() {
  ...
  async function endAuction(nft){
    ...
    const price = ethers.utils.parseUnits(nft.auction.toString(), 'ether')
    console.log(price)
    const transaction = await contract.createAuctionSale(nftaddress, nft.tokenId, {
      value:price
    })
            ...
            nfts.map((nft, i) => (
              <div key={i} className="border shadow rounded-xl overflow-hidden">
                ...
                <div className="p-4 bg-black">
                  ...
                  <button className="w-full bg-green-500 text-black font-bold py-2 px-12 rounded" onClick={() => endAuction(nft)}>End auction</button>
              </div>
            ))
          }
        </div>
